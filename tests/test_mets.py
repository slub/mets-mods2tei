...
def test_mappings(subtests, datadir):
    ...
    with subtests.test("Check ALTO linkage"):
        assert (
            mets.get_alto("PHYS_0005")
            == "https://digital.slub-dresden.de/data/kitodo/LoskGesc_497166623/LoskGesc_497166623_ocr/00000005.xml"
        )

    with subtests.test("Check IMG linkage"):
        assert (
            mets.get_img("PHYS_0005")
            == "https://digital.slub-dresden.de/data/kitodo/LoskGesc_497166623/LoskGesc_497166623_tif/jpegs/00000005.tif.medium.jpg"
        )
...
def test_data_assignment(subtests, datadir):
    ...
    with subtests.test("Check main title"):
        assert (
            mets.get_main_title()
            == "Geschichte der Mission der evangelischen Brüder unter den Indianern in Nordamerika"
        )
    ...
    with subtests.test("Check subtitle(s)"):
        assert mets.get_sub_titles() == [
            "ein Führer für Reisende; mit Kartenbeilagen und Illustrationen in Holzschnitt"
        ]
...
def test_mappings_only_phys(subtests, datadir):
    ...
    with subtests.test("Check ALTO linkage"):
        assert (
            mets.get_alto("PHYS_0005")
            == "https://digital.slub-dresden.de/data/kitodo/BurgAbha_1852685697/BurgAbha_1852685697_ocr/00000005.xml"
        )

    with subtests.test("Check IMG linkage"):
        assert (
            mets.get_img("PHYS_0005")
            == "https://digital.slub-dresden.de/data/kitodo/BurgAbha_1852685697/BurgAbha_1852685697_tif/jpegs/00000005.tif.original.jpg"
        )
