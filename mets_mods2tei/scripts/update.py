...
from ocrd_models.constants import NAMESPACES as NS, TAG_METS_FLOCAT, TAG_METS_AGENT, TAG_METS_METSHDR
...
class WorkspaceCtx:
    def __init__(self, directory, mets_url=None, automatic_backup=None):
        ...
        self.directory, self.mets_url, self.mets_basename, _ = self.resolver.resolve_mets_arguments(
            directory, mets_url, None, None
        )
...
@click.option(
    '-g', '--page-id', help="ID of the physical page (or empty if all pages)", default=None, metavar='PAGE_ID'
)
@click.option(
    '-p',
    '--path-names',
    help="how to generate local path names (from URL or from fileGrp, file ID and suffix)",
    show_default=True,
    type=click.Choice(['URL', 'GRP/ID.SUF']),
    default='URL',
)
@click.option(
    '-u',
    '--url-prefix',
    help="URL prefix to remove from path before storing downloaded files (to avoid creating host directories)",
    required=False,
)
@click.option(
    '-r',
    '--reference',
    help="whether and how to update the FLocat reference in METS",
    show_default=True,
    type=click.Choice(['no-change', 'replace-by-local', 'insert-local', 'append-local']),
    default='no-change',
)
@pass_workspace
def download_cli(ctx, file_grp, page_id, path_names, url_prefix, reference):
    ...
    """
    workspace = Workspace(
        ctx.resolver, ctx.directory, mets_basename=ctx.mets_basename, automatic_backup=ctx.automatic_backup
    )
...
@click.option(
    '-m',
    '--mimetype',
    help="Media type of the file. Guessed from extension if not provided",
    required=False,
    metavar='TYPE',
)
...
@click.option(
    '-u',
    '--url-prefix',
    help="URL prefix to add to path before removing references (or else search verbatim file refs)",
    required=False,
)
...
@click.option(
    '-u',
    '--url-prefix',
    help="URL prefix to add to path before storing references (or else keep local file refs)",
    required=False,
)
@click.argument('path', type=click.Path(exists=True, dir_okay=False))
@pass_workspace
